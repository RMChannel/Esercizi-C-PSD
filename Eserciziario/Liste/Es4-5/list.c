...
void minimoLista(List list) {
	struct node *p, *max;
	for (p=list->head, max=list->head; p!=NULL; p=p->next){
		if(cmpItem(max->value,p->value)>0) {
			max=p;
		}
	}
	Item item=max->value;
	removeListItem(list,max->value);
	addHead(list,item);
}

void massimoLista(List list) {
	struct node *p, *min;
	for (p=list->head, min=list->head; p!=NULL; p=p->next){
		if(cmpItem(min->value,p->value)<0) {
			min=p;
		}
	}
	Item item=min->value;
	removeListItem(list,min->value);
