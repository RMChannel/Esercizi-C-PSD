...
void insertNodeCalc(BTree tree, Item item, int *done) {
if (*(done)) return;
else if (isEmptyTree(tree->left)) {
	tree->left=buildTree(NULL,NULL,item);
	*(done)=1;
}
else if(!isEmptyTree(tree)){
    insertNodeCalc(tree->left,item,done);
    insertNodeCalc(tree->right,item,done);
  }
}

void insertNode(BTree tree, Item item) {
	int *n=malloc(sizeof(int));
	*n=0;
	insertNodeCalc(tree, item, n);
}