...
float calcolaEspressione(BTree tree) {
    char *value=getBTreeRoot(tree);
    if (isOperation(value)) {
        if ((*value)=='+') return calcolaEspressione(getLeft(tree))+calcolaEspressione(getRight(tree));
        else if ((*value)=='*') return calcolaEspressione(getLeft(tree))*calcolaEspressione(getRight(tree));
        else if ((*value)=='/') return calcolaEspressione(getLeft(tree))/calcolaEspressione(getRight(tree));
        else if ((*value)=='-') return calcolaEspressione(getLeft(tree))-calcolaEspressione(getRight(tree));
    }
...
int main(int argc, char *argv[]) {
    ...
    while(fgets(line,sizeof(line),fp)) {
        ...
        printf("        Valore espressione: %.2f\n",calcolaEspressione(operation));
    }
    return 0;
}